...
export default function Speed({speeds, setForm}) {
  ...
  return(
    <Fragment>
      <div className='border-2 p-8 rounded-xl'>
        <h2 className="text-xl font-bold">Suretler qutusu</h2>
        <div className='mt-3 flex flex-col'>
          <div className='max-w-[300px] mt-2 flex flex-col gap-3'>
            <Label style={{ marginBottom: '-10px' }}>Suretler qutusu</Label>
            ...
            <div className='flex items-center justify-between gap-2'>
              <input
                ...
                placeholder='Suretler qutusu'
              />
            </div>
              
            <button
              onClick={() => setForm({speed: speed}, 'speeds')}
      ...
      <div>
        <h2 className="text-2xl font-bold mt-6">Qutular</h2>
        <table className="table-fixed border border-collapse bg-white mt-2">
          ...
          {
            speeds.map(item => (
              <MetadataList key={item._id} data={item} type={'speed'} />
            ))
          }
